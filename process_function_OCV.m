...
function model=process_function_OCV(data,cellID,minV,maxV,savePlots)
  ...
  % ------------------------------------------------------------------
  SOC = 0:0.001:1; % output SOC points for this step
  filedata = zeros([0 length(data)]);
  eta = zeros(size(filetemps)); % coulombic efficiency
  Q   = zeros(size(filetemps)); % apparent total capacity
  k = ind25;

  totDisAh = data(k).script1.disAh(end) + ... % compute total discharge ampere hours
             data(k).script3.disAh(end);
  totChgAh = data(k).script1.chgAh(end) + ... % compute total charge ampere hours
             data(k).script3.chgAh(end);
  %disp(totDisAh)
  %disp(totChgAh)
  eta25 = totDisAh/totChgAh; eta(k) = eta25;% 25 degC coulombic efficiency
  data(k).script1.chgAh = data(k).script1.chgAh*eta25; % adjust charge Ah in all scripts
  ...
  Q25 = data(k).script1.disAh(end) - data(k).script1.chgAh(end);
  Q(k) = Q25;
  ...
  for k = not25',    
    ...
    Q(k) = data(k).script1.disAh(end)- data(k).script1.chgAh(end);
...
tiledlayout(rows, cols, 'TileSpacing', 'compact', 'Padding', 'compact');
    ...
    for k = 1:numtemps
        ...
        xlim([0, 100]);
        title(sprintf('%s OCV relationship at temp = %d°C', cellID, filedata(k).temp));
    ...
    if savePlots
        if ~exist('OCV_FIGURES', 'dir')
            mkdir('OCV_FIGURES');
        end
        filename = sprintf('OCV_FIGURES/%s_OCV_grid.png', cellID);
