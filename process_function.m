...
function model=process_function(data,cellID,minV,maxV,savePlots)
  ...
  % ------------------------------------------------------------------
  SOC = 0:0.05:1; % output SOC points for this step
  ...
  for k = 1:numtemps, 
    ...
    err = filedata(k).rawocv - ...
          OCVfromSOC_function(SOC,filedata(k).temp,model);
